...
extern int fts_gesture_enabled;    /* module switch */
extern bool fts_irq_enable;

extern bool FTSDebugMode;
...
static struct workqueue_struct *focaltech_usb_wq;
bool focaltech_touchpanel = false;
#endif
/*******************************************************************************
...
static irqreturn_t fts_ts_interrupt(int irq, void *dev_id)
{
	...
	FTS_TP_DBG("%s .... \n",__func__);
	if (!fts_ts) {
		pr_err("======[FTS] %s: Invalid fts_ts ======\n", __func__);
		return IRQ_HANDLED;
	}
	
	if((0 == fts_gesture_enabled) && (fts_ts->suspended == 1)){
		printk("[FTS] fts_ts_interrupt return irq .\n ");
		return IRQ_HANDLED;
	}
...
*/
	...
	if (gpio_is_valid(data->pdata->irq_gpio)) {
		gpio_request(data->pdata->irq_gpio,"fts_irq_gpio");
	...
	*/
	...
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		gpio_request(data->pdata->reset_gpio,"fts_reset_gpio");
	...
	*/
#if defined(USB_CHARGE_DETECT)
	queue_work(focaltech_usb_wq, &data->usb_chg_work);
#endif
...
static int fts_ts_stop(struct device *dev)
{
	...
	*/
	...
	#if FTS_GESTRUE_EN
	printk("== [FTS] fts_gesture_enabled: %d ==\n",fts_gesture_enabled);
	if(fts_gesture_enabled)
	{
		/*
		if(0 == fts_irq_enable)
		...
		*/
		...
		printk("[FTS] Working in doze mode!\n");
		data->suspended = true;
...
pwr_off_fail:
	...
	if(0 == fts_irq_enable)
	{	
		printk("[FTS] %s pwr off fail, enable irq.\n",__func__);
		enable_irq(data->client->irq);
		fts_irq_enable = 1;
	}
	*/
...
//	int err;
	...
	if (data->suspended) {
		dev_info(dev, "Already in already suspend state..\n");
		return 0;
	}
	data->suspended = true;
...
int fts_ts_resume(struct device *dev)
{
	...
	printk("[FTS] enter resume!\n");
	if (!data->suspended) {
		dev_info(dev, "[FTS] tochpanel already reume state ..\n");
		return 0;
	}
	data->suspended = false;
	...
	if(fts_gesture_enabled)
	{
	     ...
	     fts_write_reg(fts_i2c_client, 0xd0, 0x00);
		  if (fts_updateinfo_curr.CHIP_ID==0x54 || fts_updateinfo_curr.CHIP_ID==0x58 || fts_updateinfo_curr.CHIP_ID==0x86)
		  {
		  	fts_write_reg(fts_i2c_client, 0xd1, 0x00);
			fts_write_reg(fts_i2c_client, 0xd2, 0x00);
			fts_write_reg(fts_i2c_client, 0xd5, 0x00);
			fts_write_reg(fts_i2c_client, 0xd6, 0x00);
			fts_write_reg(fts_i2c_client, 0xd7, 0x00);
			fts_write_reg(fts_i2c_client, 0xd8, 0x00);
		  }
...
#if defined(FTS_TP_GLOVE)
	mutex_lock(&data->glove_lock);
	if (data->glove_id)
	{
		msleep(15);
		//fts_write_reg(fts_i2c_client, 0xc1, 0x01);
		txbuf[0] = 0xc1;
		txbuf[1] = 0x01;		
		fts_i2c_write(fts_i2c_client, txbuf, sizeof(txbuf));
		printk("%s:write plip reg\n", __func__);
	}
...
static int fts_ts_i2c_test(struct i2c_client *client)
{
	int rc;
	char temp;
	struct i2c_msg msgs[] =
	...
	printk(" fts_ts_i2c_test --client addr : %x ***\n", client->addr);

	rc = i2c_transfer(client->adapter, msgs, 1);
	if( rc < 0 )
	{
		printk("*** fts_ts_i2c_test error %d ***\n", rc);
		return rc;
	}
 ...
 * Return: 0
 ***********************************************************************/
 static int force_reset_guitar(void)
 {
	int err;
	printk("[FTS] %s\n", __func__);
	...
	disable_irq(fts_wq_data->client->irq);
	 ...
	 if (fts_wq_data->pdata->power_on) {
		err = fts_wq_data->pdata->power_on(false);
		if (err) {
			dev_err(&fts_wq_data->client->dev, "power off failed");
			goto pwr_off_fail;
		}
	} else {
		err = fts_power_on(fts_wq_data, false);
		if (err) {
			dev_err(&fts_wq_data->client->dev, "power off failed");
			goto pwr_off_fail;
		}
	...
	if (fts_wq_data->ts_pinctrl) {
		err = pinctrl_select_state(fts_wq_data->ts_pinctrl,
					fts_wq_data->pinctrl_state_suspend);
		if (err < 0)
			dev_err(&fts_wq_data->client->dev, "Cannot get suspend pinctrl state\n");
	}
	...
	if (fts_wq_data->pdata->power_on) {
		err = fts_wq_data->pdata->power_on(true);
		if (err) {
			dev_err(&fts_wq_data->client->dev, "power on failed");
			goto pwr_off_fail;
		}
	} else {
		err = fts_power_on(fts_wq_data, true);
		if (err) {
			dev_err(&fts_wq_data->client->dev, "power on failed");
			goto pwr_off_fail;
		}
	...
	if (fts_wq_data->ts_pinctrl) {
		err = pinctrl_select_state(fts_wq_data->ts_pinctrl,
				fts_wq_data->pinctrl_state_active);
		if (err < 0)
			dev_err(&fts_wq_data->client->dev, "Cannot get active pinctrl state\n");
	}
 ...
 ***********************************************************************/
   ...
   //  u8 check_91_reg_flag = 0;
	...
	FTS_TP_DBG("%s\n", __func__);
     ...
     for (i = 0; i < 3; i++) {
         ...
         if (ret < 0) {
            dev_err(&fts_i2c_client->dev,"Read value of 0xA3 failed!");
             //return ret;
         }
     ...
     if (i >= 3) {
        force_reset_guitar();
        dev_err(&fts_i2c_client->dev,"i >= 3  ret = %d	A3_Reg_Value = 0x%02x\n ", ret, data);
        reset_flag = 1;
     ...
     if (ret < 0) {
         dev_err(&fts_i2c_client->dev,"Read value of 0x8F failed!");
         //return ret;
     }
 
     FTS_TP_DBG("0x8F:%d, count_irq is %d\n", data, count_irq);
     flag_error = 0;
     ...
     if(1 == flag_error) {
        dev_err(&fts_i2c_client->dev,"1 == flag_error...data=%d	count_irq = %d\n ", data, count_irq);
        force_reset_guitar();
        reset_flag = 1;
     ...
     if (ret < 0) {
         dev_err(&fts_i2c_client->dev,"Read value of 0x91 failed!");
         //return ret;
     }
 
     FTS_TP_DBG("0x91 register value = 0x%02x, old value = 0x%02x\n", data, g_old_91_Reg_Value);
     ...
     if(0x01 == g_first_read_91) {
         ...
         if(g_old_91_Reg_Value == data) {
             ...
             FTS_TP_DBG("g_91value_same_count = %d\n", g_91value_same_count);
 
             if(RESET_91_REGVALUE_SAMECOUNT == g_91value_same_count) {
                 dev_err(&fts_wq_data->client->dev,"g_91value_same_count = %d\n", RESET_91_REGVALUE_SAMECOUNT);
                 force_reset_guitar();
 ...
 FOCAL_RESET_A3_REGISTER:
     ...
     if (ret < 0) {
         dev_err(&fts_i2c_client->dev,"Write value of 0x8F failed!\n");
         //return ret;
     }
...
#if defined(USB_CHARGE_DETECT)
 void focaltech_enable_change_scanning_frq(void)
{
...
EXPORT_SYMBOL(focaltech_enable_change_scanning_frq);
void focaltech_disable_change_scanning_frq(void)
{
	struct fts_ts_data *data;
...
EXPORT_SYMBOL(focaltech_disable_change_scanning_frq);
void focaltech_change_scanning_frq_switch(struct work_struct *work)
{
	u8 charger_in_flag = 0;
	struct fts_ts_data *data;

	//printk(KERN_ERR"[TPD]%s, %d\n", __FUNCTION__, __LINE__);
	data = container_of(work, struct fts_ts_data, usb_chg_work);
	...
	if (data->suspended)
	{
      ...
      if(fts_i2c_client==NULL)
          return ;
	fts_read_reg(fts_i2c_client, 0x8b, &charger_in_flag);
	//printk(KERN_ERR"charger_in_flag  = %d \n",charger_in_flag);
	if(charger_in_flag != data->charger_in)
	{
		printk(KERN_ERR"[FTS]%s: Write %d to 0x8b\n", __FUNCTION__, data->charger_in);
		fts_write_reg(fts_i2c_client, 0x8b, data->charger_in);
	}
}
#else
 void focaltech_enable_change_scanning_frq(void)
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = fts_ts_i2c_test(client);
	if(err < 0)
	{
		printk("**** fts i2c test fail! ****\n");
		return -EINVAL;
	}
...
*/
	...
	data->ts_workqueue = alloc_ordered_workqueue(FTS_WORKQUEUE_NAME, 0);
...
#if defined(USB_CHARGE_DETECT)
    focaltech_usb_wq = create_singlethread_workqueue("focaltech_wq");
    if (!focaltech_usb_wq)
    {
        printk("Creat focaltech_usb_wq workqueue failed. \n");
        return -ENOMEM;
    }
	INIT_WORK(&data->usb_chg_work, focaltech_change_scanning_frq_switch);
#endif	
	...
	focaltech_touchpanel = true;
	printk("[FTS] %s success!!\n",__func__);
