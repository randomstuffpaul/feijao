...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    GTP_INFO("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE,
	...
	GTP_INFO("Gesture: 0x%02X, key_code: %d, points: %d", doze_buf[0], key_code ,doze_buf[1]);
	if(0xcc == doze_buf[0])
	{
		GTP_INFO("Double click to light up the screen!");
	}
