...
/*******************************************************************************
*  Name: gtp_pinctrl_init
*  Brief:
*  Input:
*  Output: 
*  Return: 
*******************************************************************************/
static int gtp_pinctrl_init(void)
{
	int retval;

	/* Get pinctrl if target uses pinctrl */
	ts_pinctrl = devm_pinctrl_get(&(gt1x_i2c_client->dev));
	if (IS_ERR_OR_NULL(ts_pinctrl)) {
		retval = PTR_ERR(ts_pinctrl);
		dev_dbg(&gt1x_i2c_client->dev,
	...
	pinctrl_state_release = pinctrl_lookup_state(ts_pinctrl,
...
static s32 gt1x_i2c_test(void)
{
	...
	//while (retry++ < 3) {
		...
		//GTP_ERROR("I2c test failed%d.", retry);
	//}
