...
/* Get tp firmware version */
static ssize_t gt1x_tpfwver_show(struct device *dev,struct device_attribute *attr, char *buffer)
{	
	s32 ret = -1;
	u8 buf[12] = { 0 };
	u32 mask_id = 0;
	...
	//3.module version
	 ...
	 gt1x_i2c_read(GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);

	// num_read_chars = sprintf(buffer, "IC VERSION:GT%s_%06X(Patch)_%04X(Mask)_%02X(SensorID)", product_id, patch_id, mask_id >> 8, sensor_id);
	num_read_chars = sprintf(buffer, "GT%s_%06X_%X_%s\n", product_id,patch_id, temp_data[0],module);
	 
	return num_read_chars;
}

static ssize_t gt1x_tpfwver_store(struct device *dev,	struct device_attribute *attr,const char *buf, size_t count)
{	
	return count;
}

static ssize_t gt1x_tp_ic_info_show(struct device *dev,
				   struct device_attribute *attr, char *buffer)
{
	...
	if(gtp_suspend == 1)
	{
		count +=snprintf(buffer + count, PAGE_SIZE - count,"NOTICE:TP was suspend or resetting!\n");
	}	
	...
	//1. Get the Chip type.	
	count +=snprintf(buffer + count, PAGE_SIZE - count,"Chip_Vendor:%s\n",
			(gt1x_chip_type == CHIP_TYPE_GT1X) ? "GT1X" : "GT2X");
	
	
	count +=snprintf(buffer + count, PAGE_SIZE - count,
			"IC VERSION:GT%s_%06X(Patch)_%04X(Mask)_%02X(SensorID)\n", product_id, patch_id, mask_id >> 8, sensor_id);

	gt1x_i2c_read(GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);
	
    count +=snprintf(buffer + count, PAGE_SIZE - count,"Config_Version:0x%02X\n", temp_data[0]);
		
	return count;
}

static ssize_t gt1x_gesture_switch_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
	
    	count +=snprintf(buf + count, PAGE_SIZE - count,"%s\n", 
             gesture_enabled ? "gesture_enable" : "gesture_disable");
		
	return count;
}

static ssize_t gt1x_gesture_switch_store(struct device *dev,
				    ...
				    const char *buf, size_t size)
{
	...
	gesture_enabled = data ? 1 : 0;

	if(0 == gesture_enabled){
		gesture_enabled = 0;
...
static ssize_t gt1x_regdata_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	char temp_data[GTP_CONFIG_MAX_LENGTH] = { 0 };
	int i,count;
	...
	if(gtp_suspend == 1)
	{
		count +=snprintf(buf + count, PAGE_SIZE - count,"[GTP] touchpanel is suspend\n"); 
		return -EINVAL;
	}
	printk("start gt1x_regdata_show\n ");

	count = gt1x_i2c_read(GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);
		GTP_INFO("I2C TRANSFER: %d", i);
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		count +=snprintf(buf + count, PAGE_SIZE - count,"0x%02X,", temp_data[i]);
...
static ssize_t gt1x_regdata_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return -EINVAL;
...
static DEVICE_ATTR(fw_ver, 0644, gt1x_tpfwver_show, gt1x_tpfwver_store);
static DEVICE_ATTR(tp_ic_info, 0644, gt1x_tp_ic_info_show, NULL);
static DEVICE_ATTR(gesture_switch, 0644, gt1x_gesture_switch_show, gt1x_gesture_switch_store);  
static DEVICE_ATTR(regdata, 0644, gt1x_regdata_show, gt1x_regdata_store); 


static struct attribute *gt1x_sysfs_attrs[] = {
	&dev_attr_fw_ver.attr, 
	&dev_attr_tp_ic_info.attr,
	&dev_attr_gesture_switch.attr,
	&dev_attr_regdata.attr,
	NULL,
};

static struct kobject *gt1x_kobj = NULL;
static struct attribute_group gt1x_attr_group = {
        .attrs = gt1x_sysfs_attrs,
};

static int gt1x_init_sysfs_node(void)
{ 
	GTP_INFO("Start to Create TP_sysfs_init.");
	
	gt1x_kobj = kobject_create_and_add("goodix_ts", &gt1x_i2c_client->dev.kobj);
	if (!gt1x_kobj){
		GTP_ERROR("Create TP_sysfs_init failed!\n");
		return -ENOMEM;
	}
	return sysfs_create_group(gt1x_kobj, &gt1x_attr_group);
}
...
/*
	for (i = 0 ;i < sizeof(buf); i++){
		GTP_INFO("GTP_REG_VERSION: %x",buf[i]);
	}
*/	
	if (ver_info != NULL) {
...
/*
			if(i2c_resume_completed){
				GTP_INFO("i2c resume completed = %d",i2c_resume_completed);
				msleep(25);
			}
			else
			{
...
static ssize_t smart_cover_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    dev->state = s;
    //gt1x_smart_cover_update_state();
    queue_work(gt1x_workqueue, &gt1x_sc_dev->sc_work);
...
static int gt1x_smart_cover_init(void)
{
	...
	INIT_WORK(&gt1x_sc_dev->sc_work, sc_work_func);
...
int gt1x_suspend(void)
{
	...
	if(gtp_suspend)
	{
		GTP_INFO("devices suspend, Suspend end...");
		return 0;
	}
	...
	GTP_INFO("== gesture_enabled: %d ==",gesture_enabled);
	if (gesture_enabled) {
		gesture_enter_doze();
		gt1x_irq_enable();
...
#endif
	{
		...
		if (gpio_is_valid(GTP_RST_PORT)) {
		  gpio_set_value_cansleep(GTP_RST_PORT, 0);
		}

		ret = gt1x_power_switch(SWITCH_OFF);
		if (ret < 0)
		{
		    GTP_ERROR("GTP power off failed.");
		}
...
int gt1x_resume(void)
{
	...
	if(!gtp_suspend)
	{
		GTP_INFO("devices not suspend, Resume end...");
		return 0;
	}
...
/* init sysfs debug bode and functions */
	gt1x_init_sysfs_node();
