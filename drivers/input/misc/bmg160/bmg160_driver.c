...
{    
     ...
     #ifdef POWER_REGULATOR_BMG    
		if(enabled) {
			printk("func=%s,enable=%d,power on bmg160 power\n",__func__,enabled);
	...
	printk("func=%s,enable=%d\n",__func__,enabled);
	
	if (client_data->enable) {
		BMG_CALL_API(set_mode)(BMG_VAL_NAME(MODE_NORMAL));

		printk("bmg160 init bandwith in bmg_enable_set\n");
	...
	#ifdef POWER_REGULATOR_BMG    
		if(!enabled) {
			printk("func=%s,enable=%d,shutdown bmg160 power\n",__func__,enabled);
			ret = bmg_power_ctl(client_data, enabled);
			if (ret){
