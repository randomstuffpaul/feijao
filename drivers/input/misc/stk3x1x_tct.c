...
//modify(add) by junfeng.zhou.sz for update the version with 533451 begin . 20140115
	...
	#define STK_LT_N_CT	20 //Modify by falin.luo for ps threshold value
	#define STK_HT_N_CT	30 //Modify by falin.luo for ps threshold value
	#define STK_LOW_PS_LT_N_CT	120
	#define STK_LOW_PS_HT_N_CT	220
	#define STK_HI_PS_LT_N_CT	240
	#define STK_HI_PS_HT_N_CT	480
...
static struct device_attribute ps_raw_data_attribute = __ATTR(ps_raw_data, 0444, stk_ps_code_show, NULL);
...
//modify(add) by junfeng.zhou.sz for delete debug info with 533451 end . 
	...
	else
	{
		...
		{
			ps_data->ps_thd_h = ps_data->min_psi + 900;
			ps_data->ps_thd_l = ps_data->min_psi + 800;
		}
		else 
		{
			ret = stk3x1x_i2c_read_data(ps_data->client, 0x11, 2, &value[0]);
			...
			ps_data->ps_thd_h = word_data + 900;
			ps_data->ps_thd_l = word_data + 800;
		}
...
//due to ps_data->psi equal to rawdata so will use the psi to judge
	if((ps_data->psi <= 90)&&(diff > ps_data->pdata->low_ps_ct_h_offset - 120+5))  //0-90   HT:ps+100 LT:ps+75
	{
		ps_data->ps_thd_h = ps_data->psi + ps_data->pdata->low_ps_ct_h_offset - 120;
		ps_data->ps_thd_l = ps_data->psi + ps_data->pdata->low_ps_ct_l_offset - 45;
		is_cali = 1;
	}
	else if((( ps_data->psi <= 200) &&( ps_data->psi > 90))&&(diff > ps_data->pdata->low_ps_ct_h_offset - 105+5))   //90-200   HT:ps+115 LT:ps+90
	{
	
		ps_data->ps_thd_h = ps_data->psi + ps_data->pdata->low_ps_ct_h_offset - 105;
		ps_data->ps_thd_l = ps_data->psi + ps_data->pdata->low_ps_ct_l_offset - 30;
		is_cali = 1;
	}
	else if((( ps_data->psi <= 500) &&( ps_data->psi > 200))&&(diff > ps_data->pdata->low_ps_ct_h_offset+5))   //200-500   HT:ps+220 LT:ps+120
	{
	
		ps_data->ps_thd_h = ps_data->psi + ps_data->pdata->low_ps_ct_h_offset;
		ps_data->ps_thd_l = ps_data->psi + ps_data->pdata->low_ps_ct_l_offset;
		is_cali = 1;
	}
	else if((( ps_data->psi <= 800) &&( ps_data->psi > 500))&&(diff > ps_data->pdata->hi_ps_ct_h_offset-210))   //500-800   HT:ps+270 LT:ps+160
	{
		ps_data->ps_thd_h = ps_data->psi + ps_data->pdata->hi_ps_ct_h_offset-210;
		ps_data->ps_thd_l = ps_data->psi + ps_data->pdata->hi_ps_ct_l_offset-80;
		is_cali = 1;
	}
	else if((( ps_data->psi <= ps_data->pdata->mmi_thd_h_limit+50) &&( ps_data->psi > 800))&&(diff > ps_data->pdata->hi_ps_ct_h_offset+5))   //800-900   HT:ps+480 LT:ps+240
	{
	
		ps_data->ps_thd_h = ps_data->psi + ps_data->pdata->hi_ps_ct_h_offset;
		ps_data->ps_thd_l = ps_data->psi + ps_data->pdata->hi_ps_ct_l_offset;
		is_cali = 1;
	}
	
	else if (ps_data->psi > ps_data->pdata->mmi_thd_h_limit+50)
	{
		if (ps_data->is_have_good_cali)
		{
			ps_data->ps_thd_h = ps_data->min_psi + 900;
