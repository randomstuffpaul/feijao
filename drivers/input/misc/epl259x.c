...
static void dial_cali_work_func(struct work_struct *work)
{
	...
	else if ((epld->psi<=epl_sensor.ps.factory.mmi_thd_h_limit) && (epld->psi>1800) && (diff>700+15))
	{
		epl_sensor.ps.low_threshold  = epld->psi + 480;
		epl_sensor.ps.high_threshold = epld->psi + 700;
		is_calied = true;
	}
	else if (epld->psi>epl_sensor.ps.factory.mmi_thd_h_limit+50 || epld->psi-epld->min_psi > 800)
	{
