		...
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	return qpnp_adc_map_temp_voltage(
	                is_scud ? adcmap_btm_threshold_scud :
			adcmap_btm_threshold_byd,
        	        ARRAY_SIZE(is_scud ? adcmap_btm_threshold_scud :
                	                adcmap_btm_threshold_byd),
			bat_voltage,
		...
		uint32_t *low_threshold, uint32_t *high_threshold)
{
	...
	rc = qpnp_adc_map_voltage_temp(
		is_scud ? adcmap_btm_threshold_scud : adcmap_btm_threshold_byd,
		ARRAY_SIZE(is_scud ? adcmap_btm_threshold_scud :
				adcmap_btm_threshold_byd),
		(param->low_temp),
		&low_output);
...
#ifdef MACH_FEIJAO
	if (of_property_match_string(node, "qcom,battery-type", "TLp025C2"))
		is_scud = true;
#endif
